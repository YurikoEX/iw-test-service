...
var Eventer = (function (_super) {
    ...
    Eventer.prototype.interceptListener = function (listener, emit, args) {
        ...
        this.interceptors.filter(function (interceptor, callback) {
            callback(null, interceptor.getEvent().getText() === emit.getText());
        }, function (e, interceptors) {
            if (interceptors.length() === 0) {
                listener.apply(instance, args);
            }
            else {
                async.mapSeries(interceptors.list(), function (interceptor, interception) {
                    interceptor.intercept(emit, args, interception);
                }, function (e, interceptions) {
                    var stopped = _.any(interceptions, function (intercepted) {
                        return intercepted.stop;
                    });
                    if (!stopped) {
                        listener.apply(instance, args);
                    }
